...
public:
    ...
    // Short rationale for estimate: Funktiota dominoi for looppi joka käy läpi affiliaatiot.
    std::vector<Connection> get_connected_affiliations(AffiliationID id);

    // Estimate of performance: O(n)
    // Short rationale for estimate: vaikka for looppeja on kaksi yhden affiliaation connectioneita on vähän
    std::vector<Connection> get_all_connections();

    // Estimate of performance: O(n)
    // Short rationale for estimate:O(V+E) missä V on yhteyksien määrä ja E on liitosten määrä.
...
private:
    ...
    struct Affiliation{
        ...
        std::map<AffiliationID,Connection> Connections;
    };
